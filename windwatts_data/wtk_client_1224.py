...
class WTKLedClient1224(client_base):
    ...
    def get_1224_data(self,
        ...
        if years is None:
            raise TypeError("At least 1 value for the Parameter 'years' must be specified.")
    ...
    def get_filtered_data_1224(self,
        ...
        if lat is None and long is None:
            raise Exception('The WTKLedClient1224 class requires latitude and longitude to be specified for queries.')
        ...
        if years:
            try:
                ...
                raise ValueError("Invalid data in 'years' parameter.")
            
        if months:
            try:
                month_list = ', '.join([str(month) for month in months])
                query += f" AND CAST(mohr / 100 AS INT) IN ({month_list})"
            except Exception:
                raise ValueError("Invalid data in 'months' parameter.")
        
        if hours:
            try:
                hour_list = ', '.join([str(hour) for hour in hours])
                query += f" AND CAST(mohr % 100 AS INT) IN ({hour_list})"
            except Exception:
                raise ValueError("Invalid data in 'hours' parameter.")
    ...
    def get_statistic_1224(self,
        ...
        if group_by_month:
            select_clause.append("CAST(mohr / 100 AS INT) AS month")
            group_by_columns.append("CAST(mohr / 100 AS INT)")
        
        if group_by_hour:
            select_clause.append("CAST(mohr % 100 AS INT) AS hour")
            group_by_columns.append("CAST(mohr % 100 AS INT)")
    ...
    def get_windspeed_statistics_1224(self,
        lat: float = None,
        long: float = None,
        height: float = None,
        group_by_year: bool = False,
        ...
        """
        Calculate windspeed statistics (Overall, Yearly, Monthly or Hourly Averages) for a specified height at a specific location.
    ...
    def fetch_windspeed_timeseries_1224(self,
                                        lat: float = None, 
                                        long: float = None, 
                                        heights: list[float] = None, 
                                        years: list[int] = None, 
                                        n_nearest = 1) -> pd.DataFrame:
        """
